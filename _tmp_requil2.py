...
def _scalar_row_scales(A, B, C):
    """Per-SCALAR-row row-equilibration (one scale per row).

    Unlike _block_row_scales (one per 3-row block, too coarse for the ~30-order
    SRV/carrier imbalance at the singular stress device), this uses the exact
    per-row max-abs across the band from the blocks alone (O(N*bw), no dense J).
    ...
    if n > 1:
        ...
        cmax = jnp.max(jnp.abs(C), axis=2)
        rmax = rmax.at[:-1].set(jnp.maximum(rmax[:-1], bmax))
        rmax = rmax.at[1:].set(jnp.maximum(rmax[1:], cmax))
    rmax = rmax.reshape(-1)
    safe = jnp.where(jnp.isfinite(rmax) & (rmax > 0), rmax, jnp.asarray(1.0, dtype=rmax.dtype))
    return 1.0 / safe

...
def test_version_strings_in_sync():
    ...
    v_py = re.search(r'version = "([0-9.]+)"', pyproj).group(1)
    v_init = re.search(r'__version__ = "([0-9a-zA-Z.]+)"', init).group(1)
    v_cff = re.search(r"^version: ([0-9.]+)", cff, re.M).group(1)
    assert v_py == v_init.split("b")[0] == v_cff, (v_py, v_init, v_cff)

...
from driftjax.numerics.banded_solve import _blocks_to_lapack_banded, _banded_to_dense, extract_blocks
from driftjax.numerics.banded_native import native_banded_solve, _dense_from_blocks
from driftjax.science.contacts import boundary_bias

def _set_native(flag):
    if flag: os.environ["DRIFTJAX_NATIVE_BANDED"]="1"
    else: os.environ.pop("DRIFTJAX_NATIVE_BANDED", None)

def _lapack_solve(A,B,C,b):
    n=A.shape[0]; ab,kl,ku = _blocks_to_lapack_banded(A,B,C)
    try:
        x = solve_banded((kl,ku), ab, b.reshape(-1)); flag=0
    except Exception:
        M=_banded_to_dense(np.array(ab),kl,ku,n); x,*_=lstsq(M,b.reshape(-1),rcond=None); flag=1
    return x.reshape(n,3), flag

def _rand_banded(n, seed):
    rs = np.random.RandomState(seed)
    A=jnp.asarray(rs.randn(n,3,3)*0.3);
    A=A.at[:,0,0].set(A[:,0,0]+6); A=A.at[:,1,1].set(A[:,1,1]+6); A=A.at[:,2,2].set(A[:,2,2]+6)
...
def _stress_jac_n40():
    ...
    dev = dj.Device(n_points=40, layers=[(2e-5, mt, 1e18), (6e-5, mat, -1e18), (2e-5, mt, 1e18)], Snl=1e7, Snr=0.0, Spl=0.0, Spr=1e7)
    s = dj.simulate(dev, dj.Sweep(vmax=1.0, n_steps=3), solver=Newton(globalization="ls", fused=False))
...
def test_native_e2e_matches_shipped():
    pvk = dj.material(Chi=3.9, Eg=1.55, eps=24.0, Nc=2.2e18, Nv=1.8e19, mn=20.0, mp=20.0, tn=1e-6, tp=1e-6, A=2e5)
    dev = dj.Device(n_points=40, layers=[(0.1e-4,pvk,1e16),(5e-4,pvk,1e13),(0.1e-4,pvk,-1e16)], Snl=1e7,Snr=1e7,Spl=1e7,Spr=1e7)
    ls = spectrum()
    _set_native(False)
    s_cb = dj.simulate(dev, dj.Sweep(vmax=1.0,n_steps=8), solver=Newton(max_steps=60, globalization="ls", fused=False), ls=ls)
    try:
        _set_native(True)
        s_na = dj.simulate(dev, dj.Sweep(vmax=1.0,n_steps=8), solver=Newton(max_steps=60, globalization="ls", fused=False), ls=ls)
    finally:
        _set_native(False)
...
def test_native_solve_is_differentiable():
    ...
    A0 = A.reshape(-1); h=1e-5; fd=jnp.zeros_like(A0)
    for k in range(0, 9*n, 7):
        e = jnp.zeros(9*n); e=e.at[k].set(1.0)
...
def _set_row_equil(flag):
    if flag: os.environ["DRIFTJAX_ROW_EQUIL"]="1"
    else: os.environ.pop("DRIFTJAX_ROW_EQUIL", None)
...
def test_row_equilibration_golden_safe():
    ...
    rel = float(jnp.linalg.norm(lam_off.reshape(-1)-lam_on.reshape(-1))/(jnp.linalg.norm(lam_off)+1e-30))

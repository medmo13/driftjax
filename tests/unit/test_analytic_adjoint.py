...
def _pn_cell(n=8, **srvs):
    ...
    kw.update(srvs)
    dev = dj.Device(
        n_points=n, layers=[(1e-4, m, 1e17), (1e-4, m, -1e17)], **kw
    )
...
def _hetero_cell(n=8):
    m1 = dj.material(
        Chi=4.5, Eg=2.4, eps=10.0, Nc=2e18, Nv=1e19,
        mn=50, mp=20, tn=1e-9, tp=1e-9, A=1e4,
    )
    m2 = dj.material(
        Chi=4.3, Eg=1.5, eps=9.4, Nc=8e17, Nv=1.8e19,
        mn=100, mp=60, tn=1e-8, tp=1e-8, A=2e4,
    )
    dev = dj.Device(
        n_points=n, layers=[(5e-5, m1, 1e17), (2e-4, m2, -1e16)],
        Snl=1e5, Snr=1e6, Spl=1e6, Spr=1e5,
    )
    return init_cell(dev.design(), spectrum(normalize=False))

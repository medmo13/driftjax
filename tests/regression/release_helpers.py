...
def pce_jax(x, vmax=1.0, n_steps=20, n_points=N_DEFAULT, max_steps=400):
    sol = simulate(
        ...
        solver=Newton(max_steps=max_steps),
        optics=BeerLambert(),
    )
...
def _flatband_work_function(Nc, Nv, Eg, Chi, doping):
    ...
    d_ef = jnp.where(doping > 0, jnp.log(jnp.abs(doping) / ni), -jnp.log(jnp.abs(doping) / ni))
    return -efi - d_ef
...
def constraints_jax(x):
    ...
    return -jnp.array(
        [
            ...
            x[8] - ABSORBER["Chi"],
            pl - x[8] - x[7],
            x[8] + x[7] - ABSORBER["Chi"] - ABSORBER["Eg"],
            ABSORBER["Chi"] - x[1],
        ]
    )
...
def iv_distance_jax(y1, y2, norm=2.0):
    ...
    return jnp.sum((jnp.abs(jnp.interp(theta, t1, r1) - jnp.interp(theta, t2, r2))) ** norm)

...
material = dj.material(
    ...
    mn=100, mp=100, Et=0, tn=1e-8, tp=1e-8, A=20000.0,
)
des = dj.Device(
...
def run(vmax=1.1, n_steps=25, alpha_mode="beer-lambert"):
    sol = simulate(
        des, Sweep(vmax=vmax, n_steps=n_steps),
        solver=Newton(), optics=BeerLambert(alpha_mode=alpha_mode),
    )
    return dict(
        ...
        voc_V=float(sol.voc),
        ff=float(sol.ff),
        vmax=vmax, n_steps=n_steps, alpha_mode=alpha_mode, n_points=N_POINTS,
    )
...
if __name__ == "__main__":
    ...
    sol = simulate(
        des, Sweep(vmax=1.1, n_steps=101),
        solver=Newton(), optics=BeerLambert(alpha_mode="beer-lambert"),
    )
    res = dict(
        eff_pct=float(sol.eff) * 100,
        jsc_mA_cm2=float(sol.jsc) * 1000.0,
        voc_V=float(sol.voc),
        ff=float(sol.ff),
        vmax=1.1, n_steps=101, alpha_mode="beer-lambert", n_points=N_POINTS,
    )
    res["script"] = "ex1_np_junction"
    v, j = np.asarray(sol.voltages), np.asarray(sol.current)
    plot_iv_curve(v, j, str(RESULTS_DIR / "ex1_iv.png"),
                  title=(f"eff={res['eff_pct']:.2f}%  Jsc={res['jsc_mA_cm2']:.1f} "
                         f"Voc={res['voc_V']:.3f} V  FF={res['ff']:.3f}"),
                  p_in=float(np.sum(np.asarray(sol.P_in, float))))
    from driftjax.viz.io import save_results

    save_results(
        {**res, "voltages": v.tolist(), "current_mA_cm2": (j * 1000.0).tolist()}
    )

...
def _material(mp, Eg):
    return material(
        Eg=jnp.asarray(Eg, float),
        Chi=3.0,
        eps=10.0,
        Nc=1e18,
        Nv=1e18,
        mn=130.0,
        mp=jnp.asarray(mp, float),
        A=2e4,
    )
...
def _device(mp, Eg):
    m = _material(mp, Eg)
    return Device(
        n_points=N, layers=[(1e-4, m, 1e17), (1e-4, m, -1e17)], Snl=1e7, Snr=0, Spl=0, Spr=1e7
    )


def get_J(mp, Eg):  # eager forward (target + plotting)
    return np.asarray(simulate(_device(float(mp), float(Eg)), Sweep(vmax=1.1, n_steps=NS)).current)
...
if __name__ == "__main__":
    ...
    res = minimize(
        ...
        options={"maxiter": int(os.environ.get("MAXITER", "50")) * 2, "ftol": 1e-9, "disp": False},
        callback=cb,
    )
    ys = np.array(ys)
    ...
    print(
        f"material recovery: R final = {r_final:.3e}  recovered (log10 mp, Eg) = "

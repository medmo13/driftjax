...
from _helpers import bt_transpose, solve_sparse_lu, equilibrate
...
def test():
    ...
    dev_h = dj.Device(n_points=15, layers=[(1e-4, m, 1e17), (1e-4, m, -1e17)], Snl=1e7, Snr=0.0, Spl=0.0, Spr=1e7)
    dev_p = dj.Device(n_points=15, layers=[(2e-5, mt, 1e18), (6e-5, m, -1e18), (2e-5, m, 1e18)], Snl=1e7, Snr=0.0, Spl=0.0, Spr=1e7)
    for label, dev in [("Homojunction", dev_h), ("Perovskite 3-layer", dev_p)]:
        ...
        for pot, vb in zip(s.potentials, s.voltages):
            ...
            r_d = float(jnp.linalg.norm(J.T @ lam_d - g) / (jnp.linalg.norm(g)+1e-30)) if np.all(np.isfinite(np.array(lam_d))) else float('inf')
            # BT
            ...
            if lam_eq_tilde is not None:
                ...
                r_eq = float(np.linalg.norm(np.array(J.T) @ lam_eq - np.array(g)) / (np.linalg.norm(g)+1e-30)) if np.all(np.isfinite(lam_eq)) else float('inf')
            else:
                r_eq = float('inf')
            try:
                ...
                c = float('inf')
            print(f"V={vb:.2f} dense r={r_d:.2e} BT r={r_bt:.2e} sparse r={r_sp:.2e} equil-sparse r={r_eq:.2e} cond {c:.2e}")

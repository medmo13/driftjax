...
def g_jax(x):
    ...
    return -jnp.array([ChiE - P0, ChiH - Chi_P, PL - ChiH - EgH,
...
if __name__ == "__main__":
    xs, ys = [], []
    def cb(xk):
        xs.append(np.asarray(xk).copy()); ys.append(_psc.pce(xk))
    res = minimize(lambda x: -_psc.pce(x), _psc.X0, method="SLSQP",
                   jac=lambda x: -_psc.grad_pce(x),
                   bounds=list(zip(_psc.vl, _psc.vu)),
    ...
    print(f"holistic: start PCE = {start:.3f}%  ->  end PCE = {ys[-1]:.3f}%  "

...
def main():
    ...
    _gj2 = jitg(X0)
    rj = time.time() - t
    print(f"grad call 1 (compile+run): {c1:.2f}s")
    print(f"grad call 2 (cached)     : {r2:.3f}s")
    print(f"jit  call 1 (compile+run): {cj:.2f}s")
    print(f"jit  call 2 (cached)     : {rj:.3f}s")
    print(f"grad[0]={float(g1[0]):.4e} (Eg1)  grad[12]={float(g1[12]):.4e} (t1)")

...
PEROV = dict(Nc=3.9e18, Nv=2.7e18, Eg=1.5, eps=10, Et=0, mn=2, mp=2, tn=tau, tp=tau, Chi=3.9, Br=2.3e-9, A=A)
Chi_P, Eg_P = 3.9, 1.5
vl = np.array([1, 1, 1, 17, 17, 0, 0, 1, 1, 1, 17, 17, 0, 0, 17, 17], float)
vu = np.array([5, 5, 20, 20, 20, 3, 3, 5, 5, 20, 20, 20, 3, 3, 20, 20], float)
X0 = np.array([1.661788237392516, 4.698293002285373, 19.6342803183675, 18.83471869026531,
...
def x2des(x, n_points=None):
    ...
    return Device(n_points=N if n_points is None else n_points,
                  layers=[(L_ETM, etm, 10**x[14]),
                          (L_Perov, material(**PEROV), 0.0),
                          (L_HTM, htm, -10**x[15])], Snl=S, Snr=S, Spl=S, Spr=S)


def pce_jax(x):
    return jnp.asarray(simulate(x2des(x), Sweep(vmax=1.0, n_steps=NS),
                                solver=Newton(max_steps=400), optics=BeerLambert()).eff) * 100.0
...
def g(x):                             # 5 band-alignment feasibility constraints
    ...
    return np.asarray(-jnp.array([ChiE - P0, ChiH - Chi_P, PL - ChiH - EgH,
                                  ChiH + EgH - Chi_P - Eg_P, Chi_P - ChiE]))
...
if __name__ == "__main__":
    ...
    def cb(xk):
        ...
        ys.append(pce(xk))
    res = minimize(lambda x: -pce(x), X0, method="SLSQP", jac=lambda x: -grad_pce(x),
                   bounds=list(zip(vl, vu, strict=True)), constraints=[{"type": "ineq", "fun": g}],
    ...
    print(f"psc: start PCE = {start:.3f}%  ->  end PCE = {-res.fun:.3f}%  "
    ...
    with open(out / "perovskite.json", "w") as fh:
        json.dump({"history": [float(v) for v in ys],
                   ...
                   "x_final": [float(v) for v in np.asarray(res.x)]}, fh, indent=2)

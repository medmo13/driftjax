...
def measure_memory_scaling(mesh_sizes: list[int] | None = None) -> dict:
    ...
    for N in mesh_sizes:
        ...
        results.append({"N": N, "dense_MB": dense_bytes / (1024**2), "banded_MB": banded_bytes / (1024**2), "ratio": dense_bytes / banded_bytes})
...
def measure_adjoint_scaling(mesh_sizes: list[int] | None = None) -> dict:
    ...
    for N in mesh_sizes:
        try:
            dev = dj.Device(n_points=N, layers=[(4e-5, mat, 1e17), (1e-4, mat, -1e15)], Snl=1e7, Snr=0, Spl=0, Spr=1e7)
...
def measure_jit_cold_warm() -> dict:
    ...
    return {
        "test": "V19_jit_cold_warm", "t_cold_s": t_cold, "t_warm_median_s": t_warm_median,
    ...
    for r in results:
        ...
        if r["test"] == "V17_memory_scaling":
            ...
            for m in r["measurements"]:
                print(f"    {m['N']:>6d} {m['dense_MB']:>10.2f} {m['banded_MB']:>10.4f} {m['ratio']:>8.1f}x")
            ...
            print(f"    Cold: {r['t_cold_s']:.3f}s  Warm: {r['t_warm_median_s']:.3f}s  Overhead: {r['overhead_ratio']:.1f}x")
    print()

...
def test_equilibrium_limit() -> dict:
    ...
    return {"test": "V8_equilibrium_limit", "np_over_ni2_error": np_error, "R_max": R_max, "passed": np_error < 1e-3 and R_rel < 1e-3}
...
def test_optical_limiting() -> dict:
    ...
    ls = spectrum()
    mat = dj.material(Chi=3.9, Eg=1.5, eps=9.4, Nc=8e17, Nv=1.8e19, mn=100, mp=100, tn=1e-8, tp=1e-8, A=1e4)
    dev = dj.Device(n_points=100, layers=[(1e-4, mat, 1e17), (1e-4, mat, -1e15)], Snl=1e7, Snr=0, Spl=0, Spr=1e7)
    cell = init_cell(dev.design(), ls)

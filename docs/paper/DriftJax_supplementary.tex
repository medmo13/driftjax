...
Implementation descriptions follow the v0.1.18 source tree described in
...
actual degeneracy range, including the branch transition. A smooth blend
$F=(1-w)Q+wS$ was found to differentiate incorrectly in an earlier
revision (missing $w'(S-Q)$ term, $\sim$2.5$\times10^{-4}$ relative error
at $\eta\approx10$); the corrected exact derivative of the implemented
blend agrees with central differences to $7\times10^{-6}$ over
$\eta\in[-20,50]$, densely sampled around the transition.
...
\subsection{Backend regimes and failure semantics}
The $(N,\kappa)$ regime map (\code{docs/paper/records/solver\_phase\_map.json},
Fig.~\ref{suppfig:phasemap}) compiles dense vs structured transpose
outcomes across all benchmark records. Regimes: dense-optimal (both OK,
dense faster as measured, not assumed), structured-competitive,
structured-kernel-wins (raw \code{dgbsv} kernel only), dense-only
(banded FAILED, dense OK), and uncertifiable (neither backend produces a
certified solution, e.g.\ the rank-deficient 3-layer perovskite transpose
at $\kappa\sim4\times10^{45}$ where dense raises singular-matrix and the
banded path flags fallback). Fallback zeros are recorded with explicit
FAILED status, never as a solution vector.
\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{solver_phase_map.png}
\caption{Adjoint solver regimes in $(N,\kappa)$ space from measured
outcomes across all records. No point yet lands in structured-optimal on
wall time: the raw kernel wins everywhere measured ($0.1$~ms vs
$2$~ms dense at $N=100$, widening with $N$), but wrapper overhead
(packing, host callback, equilibration, residual check) dominates the
structured path end to end.}
\label{suppfig:phasemap}
\end{figure}
...
coupled-solver order; distinct from the main-text five-level study plus
$N=2000$ extension (main Section~4.4), which is the authoritative mesh
evidence \\
Homojunction cross-code & PCE 19.9792\% (d$\partial$V) versus 20.0002\% (v0.1.18);
$\max|\Delta J|=4.2\times10^{-5}$~A\,cm$^{-2}$ & Curve revisions and
physical inputs must be matched \\
CdS/CdTe cross-code & PCE 13.3076\% (d$\partial$V) versus 13.3132\% (v0.1.18);
...
\begin{table}[tbp]
\centering\small
\fbox{\parbox{0.92\linewidth}{\centering\small\bfseries SUPERSEDED --- DO NOT CITE\\
Timing pairs retained for provenance only; see main Section~6.1.}}
\vspace{4pt}
\caption{Timing pairs retained from the supplied reports
for provenance only. The reference for the first four rows is labelled
...
hardware, tolerances, and code revisions are withdrawn (see main
Section~6.1).}
\label{supp:timings}
...
A controlled A/B timing study (3-run averages, identical device, same venv, CPU x64)
confirms the implementation-level speedups in Table~\ref{tab:deltapv-comparison} (defined in the main text):
PN junction 5.3$x$ faster (41.7~s vs 7.8~s), heterojunction 3.6$x$ faster
(43.6~s vs 12.1~s), and perovskite PSC 3.4$x$ faster (46.7~s vs 13.6~s).
For gradient evaluation, the implicit adjoint adds 3~s over the forward solve
(57.8~s vs 56.9~s), while a 4-point finite-difference gradient of the
reference cost $\sim$142~s (2.2$x$ slower). Results are archived with
per-bias logs in \code{docs/paper/records/research\_15\_deltapv\_crosscode.json}.
...
The following table identifies, for each backend: default or experimental status, Python versus XLA control flow, callback use, pivoting strategy, differentiability, fallback behavior, and supported hardware.

\begin{papertable}
\centering
\caption{Backend matrix: DriftJax v0.1.18 solver backends.}
...
were machine-counted on the v0.1.16 release commit. The v0.1.18 review-fix revision
...
agreement), and the review-fix revision adds
\code{tests/unit/test\_independent\_gradient\_reference.py} (4 tests:
closed-form ultimate-efficiency sensitivity identity, finite-difference
comparison, physical monotonic decay, and a Richardson-refined check),
bringing the machine-counted total to 283 (\code{pytest --collect-only}
This revision resolves the major scientific review observations: (1) a backend matrix
Table~\ref{tab:backend-matrix} clarifies that the forward Newton solve is callback-free
while the adjoint transpose path retains scipy callbacks; (2) the native solver is described
as row-equilibrated block Thomas GE (not pivoted banded GE); (3) fallback semantics are
formally defined: singular rank-deficient states return no certified gradient.

on the v0.1.18 review-fix tree: unit 197, regression 37, gradient 9,
property 14, literature 22, validation 4, reproducibility 3,
conservation 4, convergence 4). The delta since 262 is the 4
test\_independent\_gradient\_reference.py tests (R3 independent gradient
reference), 3 tests/regression/test\_three\_layer\_stress.py tests
(R-audit pinning the singular stress device's reproducible converge/diverge
behavior and non-physicality). The review-fix revision (v0.1.18) also adds 19 tests: test\_banded\_ge.py (7 tests) + updated test\_banded\_native.py (6 tests); total banded solver tests: 32 native-banded-solver correctness and differentiability tests (DRIFTJAX\_NATIVE\_BANDED=1), verified to match LAPACK to 1e-15 and jax.grad to central-FD; suite is green: 271 passed, 1 xfailed, 0 failed.

The material inventory is documented as 25 entries; a run should store the
...
\code{validation/gradient\_evidence\_hetero.py} introduced in v0.1.18.

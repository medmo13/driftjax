...
Implementation descriptions follow the supplied v0.1.15 release documents;
the source tree and original run logs were not available for an
independent executable audit. No additional simulation data are introduced.
...
\begin{table}[tbp]
\centering\small
\caption{Timing pairs retained from the supplied reports. Ratios are
computed from those pairs. The reference for the first four rows is
labelled $\partial\mathrm{PV}$ v0.0.5; the last row compares against
an eager DriftJax implementation. These are not new measurements.}
\label{supp:timings}
\begin{tabularx}{\linewidth}{@{}Xrrr@{}}
\toprule
Workload & Reference (s) & DriftJax (s) & Ratio \\
\midrule
Homojunction forward sweep & 77.1 & 0.26 & $297\times$ \\
CdS/CdTe forward sweep & 52.8 & 0.26 & $203\times$ \\
Two-parameter gradient & 58.5 & $\sim0.8$ & $\sim73\times$ \\
Two-parameter SLSQP & 426.2 & $\sim37$ & $\sim11.5\times$ \\
Transient, 50 steps & 74.3 & 0.55 & $135\times$ \\
\bottomrule
\end{tabularx}
\end{table}
...
The v0.1.15 release notes report 31 tests in the fast core suite; the
full v0.1.14 test suite includes 204 unit/regression tests and 64 smoke
tests. These counts refer to different collection scopes (fast core versus
full suite) and should not be compared directly. The material inventory
is documented as 25 entries, while earlier manuscripts listed 13; a run
should store the actual material data rather than rely on that inventory
count. The example counts also differ across documents, so example
identifiers are used without asserting a fixed validated total.

All eleven raster figures were recovered from the original supplied
compiled manuscript during the earlier revision. The present package
uses five in the shared main/CAS article and six in this supplement.
The image pixels, numerical curves, and embedded version labels are
unchanged. A final reproducible release should regenerate the figures
from archived arrays and reconcile the remaining material, normalisation,
...
use full spectrum \\
\addlinespace
SLSQP vs Nelder--Mead & Re-run example 13; verify optimizer identity &
Confirmed Nelder--Mead, 165 evals, MSE $4.2\times10^{-6}$ & Attribution
corrected in text and figure \\
\addlinespace
...
\addlinespace
DGSM sampling stability & 32/64/128 Sobol samples; Spearman rank
correlation & Top-3 parameters identical; $\rho=0.73$ (32 vs 128) &
Section 5.1 updated with sampling convergence \\
\bottomrule
\end{tabularx}
\end{papertable}

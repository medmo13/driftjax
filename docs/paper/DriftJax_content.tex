...
\subsection{Structured-solver diagnosis and the v0.1.17 switch}
...
$\leq30$ steps, warm, mean of 10) costs 30.7~ms at $N=50$, 35.2~ms at
$N=100$, 37.2~ms at $N=200$, and 47.8~ms at $N=400$: near-flat scaling
consistent with fixed JIT/host-callback overhead dominating the
$O(N\cdot\mathrm{kl}\cdot\mathrm{ku})$ factorisation at these sizes.
A complete forward sweep at $N=100$ takes 57.6~ms (10 biases) and
111.8~ms (20 biases) warm. The adjoint gradient (forward plus backward)
at $N=100$ with 10 biases takes $\sim$1.0~s warm, approximately
$17\times$ the corresponding forward sweep, consistent with the
$O(KN^3)$ dense-adjoint accounting. These are single-architecture
...
30.7--47.8~ms warm from $N=50$ to $N=400$ (near-flat scaling: fixed
JIT/host-callback overhead dominates at these sizes). The adjoint
gradient costs approximately $17\times$ the forward sweep at $N=100$.
Dense Jacobian storage is $167\times$ larger than block storage.
Historical cross-implementation timing ratios are withdrawn; only the
controlled single-architecture measurements above are retained.

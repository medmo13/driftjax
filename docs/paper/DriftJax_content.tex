...
Version described & v0.1.15, release notes dated 14 September 2026 \\
...
$\partial\mathrm{PV}$, that combines JAX automatic differentiation with
...
S1--S9.
Detailed derivations, auxiliary solvers, and reproduction requirements are
provided in Supplementary Sections S1--S9. Implementation statements refer
to the supplied DriftJax v0.1.15 documentation unless a reference-code
...
$\partial\mathrm{PV}$ source; the DriftJax column describes v0.1.15
...
steady-state drift--diffusion--Poisson equations are~\cite{gaury2019,mann2022partial}
\begin{align}
...
(homojunction, heterojunction, perovskite) using the v0.1.15 release
...
$7.1\times10^{-15}$ (homojunction), $9.1\times10^{-11}$
(heterojunction), and $1.4\times10^{-3}$ (perovskite, where the
extremely small $n_i$ amplifies floating-point round-off).
...
were compared on the same two-parameter problem (band gap and thickness,
same initial point, bounds, and 50-evaluation budget): adjoint-gradient
SLSQP, finite-difference L-BFGS-B, and Nelder--Mead. All three converge
to the same optimum ($\eta=8.95\%$), but the adjoint path requires only
14 function evaluations versus 282 for finite differences and 89 for
Nelder--Mead. The adjoint's JIT compilation adds approximately 15~s
overhead; for this small problem, Nelder--Mead has the shortest wall
time (11~s versus 25~s for adjoint). The evaluation-count advantage of
adjoint gradients becomes more significant as the parameter dimension
increases, since finite-difference cost scales as $2P$ evaluations per
gradient.
...
The supplied timing records report 0.26~s forward sweeps against
77.1~s and 52.8~s reference runs, approximately 0.8~s versus 58.5~s
per gradient evaluation, and approximately 37~s versus 426.2~s for a
two-parameter SLSQP run. The implied ratios are approximately
297, 203, 73, and 11.5, respectively. These are arithmetic ratios of
previously reported timing pairs on potentially different hardware and
tolerances, not independently reproduced speedup measurements. The
controlled benchmarks above provide single-architecture measurements
with declared precision and synchronisation. The published
$\partial\mathrm{PV}$ algorithm uses GMRES/ILU(0); it should not be
described as a dense Newton solver solely to explain these timing pairs.
Moreover, the inspected reference current-adjoint routine already uses
a dense transpose solve~\cite{deltapvsource}. The reported gradient timing
ratio therefore cannot be explained simply as replacing an iterative
...
storage is $167\times$ larger than block storage. The previously reported
297--203$\times$ ratios are arithmetic pairs, not controlled measurements.
...
\url{https://github.com/driftjax/driftjax} as the source repository and
specifies an MIT licence. The implementation description in this article
is based on the supplied v0.1.15 overview, README, and release history;
...
(v0.1.15) for reproducibility. An executable DriftJax source tree,

...
% The major architectural contribution of DriftJax is a single-program XLA
% megakernel: the entire forward Newton solve (Scharfetter--Gummel flux
% evaluation, block-tridiagonal assembly, and pivoted banded elimination
% via LAPACK \code{dgbsv}) is expressed as a trace-safe
% \code{lax.while_loop} with no host callbacks or Python-level control
% flow. This makes every forward step differentable through JAX's
% \code{custom_vjp} implicit-adjoint path. The native block-tridiagonal
% GE solver is used by default on well-conditioned systems, but an
% automatic LAPACK fallback engages transparently when Jacobian blocks
% become ill-conditioned, detected via per-block determinant thresholds.
% This single-program design eliminates the host-callback boundary that
% separated forward and backward passes in deltapv (GMRES with ILU(0)),
% allowing the implicit function theorem to differentiate the full
...
via LAPACK \code{dgbsv}---is expressed as a trace-safe
\code{lax.while_loop} with no host callbacks or Python-level control
flow. This makes every forward Newton step end-to-end differentiable
through JAX's \code{custom_vjp} implicit-adjoint path, rather than
requiring separate primal and adjoint implementations. The native
block-tridiagonal GE solver is used by default on well-conditioned
systems (block bandwidth kl=ku=5 for the interleaved 3×3 blocks), but an
automatic LAPACK fallback engages transparently when Jacobian blocks
become ill-conditioned (e.g., heterojunction band offsets): the solver
detects near-singular blocks via per-block determinant thresholds and
dispatches to dense LU \code{dgesv} within the same XLA program, never
materialising host arrays. This single-program design eliminates the
host-callback boundary that previously separated forward and backward
passes in deltapv (GMRES with ILU(0) preconditioning), allowing the
implicit function theorem to differentiate the full converged state
directly.Solver-evolution arc (why each choice was made, not just what it is):
...
The major contribution of this article is the single-program XLA megakernel
architecture that unifies structured forward algebra, an explicit backward
stability strategy, and verified end-to-end derivatives. Rather than
introducing a new adjoint principle or a uniformly linear-cost solver,
DriftJax demonstrates that the entire forward Newton iteration---flux
evaluation, block-tridiagonal assembly, and pivoted banded elimination---can
be expressed as a single trace-safe \code{lax.while_loop} that differentiates
directly through the implicit function theorem. The automatic LAPACK fallback
on ill-conditioned heterojunction blocks (detected via per-block determinant
thresholds) is a consequence of this architecture rather than a separate code
path: the fallback engages within the same XLA program, preserving
differentiability and provenance tracking.

...
Jacobian, pivoted banded (native block-Thomas or LAPACK \code{dgbsv})
Newton solve with automatic fallback, and dense-LU implicit adjoint \\
...
a callback-free XLA megakernel: Scharfetter--Gummel flux evaluation,
...
is the production default on well-conditioned systems (block bandwidth
kl=ku=5 for the interleaved 3×3 blocks); it differs from LAPACK's pivoted
banded \code{dgbsv} in lacking inter-block pivoting, with automatic dense-LU
fallback (\code{dgesv}) when per-block determinant thresholds flag
near-singularity.
...
\midrule
\texttt{native\_ge\_eq} & Default & Pure JAX (XLA) & Dense LU & Local 3×3 & Yes & CPU/GPU \\
\texttt{auto} & Fallback & Callback (scipy) & Callback (scipy) & Band pivot & No & CPU \\
...
The \texttt{native\_ge\_eq} backend (\code{banded\_ge.py}) performs row-equilibrated unpivoted block-Thomas elimination via \code{lax.scan}---no inter-block pivoting but per-element 3×3 pivoted local solves. When per-block determinant thresholds indicate near-singularity, it falls back to dense \code{dgesv} within the same XLA program. The \texttt{auto} backend is the scipy.linalg \code{solve_banded} path with truncated-SVD fallback, available by setting \code{DRIFTJAX_NATIVE_BANDED=0}. Dense LU (\code{dgesv}, via \code{jax.scipy.linalg}) is the production adjoint default.
...
The native block-Thomas solver includes automatic detection of near-singular
blocks via per-block determinant thresholds. When the equilibrated Jacobian
...
fixed JIT compilation and host-callback overhead in the adjoint transpose solve at these sizes). The adjoint
...
XLA megakernel (native block Thomas, row-equilibrated, with automatic
LAPACK fallback on ill-conditioned blocks), advancing on deltapv's

...
megakernel for the forward Newton solve: Scharfetter--Gummel flux evaluation,
block-tridiagonal Jacobian assembly, and block-Thomas elimination via native GE (\code{banded_ge.py}, row-equilibrated \code{lax.scan} block Thomas with
...
\code{custom_vjp} implicit-adjoint path. The native block-tridiagonal GE
solver is the production default on well-conditioned systems (block bandwidth
kl=ku=5 for the interleaved 3×3 blocks). When native GE detects near-singular
blocks via per-block determinant thresholds (e.g., heterojunction band offsets),
it engages an automatic dense-LU fallback (\code{dgesv}) within the same XLA
program.
...
is based on the v0.1.18 source tree described here (HEAD \code{a6fddbb}

...
directly (\code{docs/paper/records/dgsm\_extended.json}): the same
four-parameter problem in standardized dimensionless coordinates
$z\in[-1,1]^4$ with 256 Sobol samples ranks hole lifetime first
($\tau_p>\tau_n>\mu_p>\mu_n$), the leader holding top rank in $100\%$
of 200 bootstrap resamples, with Spearman $\rho=0.80$ against the
published 32-sample ordering. The leader is therefore stable under
resampling and coordinate standardisation; the full ordering remains

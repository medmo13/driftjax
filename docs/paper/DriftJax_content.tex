...
rounding floor at $h\leq10^{-6}$. Adjoint gradients are mesh-stable
...
$V_{\mathrm{oc}}$ (interpolated) & --- & --- & --- & Not performed \\
...
A four-arm causality study on identical saved Jacobians (homojunction
and heterojunction, $N=100$, $V=0.5$~V) separates the roles of scaling
and algorithm: unpivoted block-Thomas vs pivoted banded, each raw and
row-equilibrated, plus dense LU, with forward errors measured against a
longdouble-residual refined reference
(\code{docs/paper/records/solver\_causality.json}). At this
non-degenerate bias all arms agree (forward errors $10^{-13}$--$10^{-27}$,
backward residuals $10^{-13}$--$10^{-8}$): row scaling alone does not
separate the solvers here, bounding the failure regime to the
equilibrium-adjacent degenerate states above rather than all biases.
Equilibration changes the banded backward residual by orders of
magnitude on the homojunction ($5.1\times10^{-12}$ raw vs
$8.7\times10^{-8}$ equilibrated) while leaving forward errors at
$10^{-14}$--$10^{-15}$---further evidence that backward residuals do
not certify solution accuracy. Row imbalance is therefore established
as a major numerical amplifier, not an isolated sole cause; the
remaining candidates (transpose assembly, right-hand-side construction,
state mismatch, contraction cancellation) are listed as open.
...
on the matched Si-like homojunction to $N=2000$
(\code{docs/paper/records/mesh\_extended.json}) gives PCE $20.0002\%$,
$20.0240\%$, $20.0378\%$ at $N=500$, $1000$, $2000$ (each $\sim7$~s wall
time), with observed order $\approx0.79$; Richardson extrapolation from
these levels lands at $20.019\%$, below the finest-grid value,
confirming the non-asymptotic regime rather than correcting it. No
clean coupled-solver order is claimed.
...
budget unconverged). Reduced resolution and budget confound the noise
...
solver. The evidence presented constitutes numerical verification and

...
\code{docs/paper/records/adjoint\_transpose\_bench.json}) locates that
crossover instead of gesturing at it. On identical matrices with
longdouble references: at $N=100$, homojunction
($\kappa=6.2\times10^{11}$), dense LU takes 1.9~ms with forward error
$5.3\times10^{-14}$ vs banded $287$~ms at $2.1\times10^{-14}$ (no
fallback); heterojunction ($\kappa=1.8\times10^{17}$), dense 1.3~ms at
$2.2\times10^{-28}$ vs banded $295$~ms at $2.3\times10^{-27}$. Accuracy
is equivalent; the $\sim$150$\times$ wall-time gap is pure host-callback
and equilibration overhead at this size. Extending along $N$ (homojunction
$N=200/400$, heterojunction $N=200$), dense times grow $8\times$ per
doubling ($1.7\to8.0\to65$~ms, cubic as expected) while banded stays
nearly flat ($185\to205\to254$~ms, overhead-dominated); fitting
$T_{\mathrm{dense}}=aN^3$ against the banded line places the crossover
at $N\approx620$. Production meshes ($N\leq500$) therefore sit below
crossover, which justifies the dense default quantitatively rather than
by habit. On the rank-deficient 3-layer perovskite transpose
($\kappa\sim4\times10^{45}$) dense LU raises a singular-matrix error
while the banded path returns with \code{used\_fallback} set---an honest
negative result bounding the structured transpose to nonsingular systems.

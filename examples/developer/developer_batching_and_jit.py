...
def main():
    ...
    rel_current = float(np.max(np.abs(j_batch - np.asarray(serial.currents))) / np.max(np.abs(serial.currents)))
    ...
    rel_generation = float(np.max(np.abs(sharded - full_generation)) / np.max(np.abs(full_generation)))

    fig, axes = new_figure(nrows=1, ncols=2, **DOUBLE_WIDE)
    axes[0].plot(serial.voltages, serial.currents * 1e3,
                 color=style.BLUE, linewidth=1.6, label="serial API")
    axes[0].plot(v_batch, j_batch * 1e3, "o", ms=5, color=style.ORANGE,
                 label="batched solver")
    axes[0].set(xlabel=style.LBL_BIAS, ylabel="current density / mA cm$^{-2}$")
    axes[0].set_xlim(0, 0.75)
    jsc_vals = [abs(float(serial.jsc)*1e3), abs(float(j_batch[0])*1e3) if len(j_batch)>0 else 15]
    y_top = max(jsc_vals)
    ...
    axes[1].bar(["first", "warm", "batch"], [first_s, warm_s, batch_s],
                color=(style.BLUE, style.GREEN, style.VIOLET), edgecolor="black", linewidth=0.5, width=0.6)
    ...
    save_json("developer_batching_and_jit", {
        "metadata": execution_metadata(), "first_call_s": first_s, "warm_call_s": warm_s,
        "batched_call_s": batch_s, "max_relative_current_error": rel_current,
        "max_relative_generation_error": rel_generation, "sharding": shard_meta,

...
def device_from(x, points, window):
    absorber = dj.material(
        Chi=3.9, Eg=x[0], eps=12.0, Nc=5e17, Nv=5e18, mn=1000.0, mp=300.0, tn=1e-6, tp=1e-6, A=3e4
    )
    return dj.Device(
        n_points=points,
        layers=[(5e-6, window, 1e18), (x[1] * 1e-4, absorber, -1e16)],
        Snl=1e7,
        Snr=1e7,
        Spl=1e7,
        Spr=1e7,
...
def main():
    ...
    window = dj.material(
        Chi=3.5, Eg=2.4, eps=10.0, Nc=2e18, Nv=2e18, mn=100.0, mp=100.0, tn=1e-7, tp=1e-7, A=1e4
    )
    ...
    axes[0].plot(
        *x_opt, marker="*", color=style.RED, mec="black", ms=14, ls="none", label="optimized"
    )
    ...
    axes[0].set_title(
        r"Feasibility map: $\eta$(E$_g$, $W$) with SLSQP start/end", fontsize=9, pad=8
    )
    ...
    axes[1].set_title(
        f"Gain: {eff_start:.1f}% → {pce_opt:.1f}% (+{pce_opt - eff_start:.1f}pp)", fontsize=9, pad=8
    )
    # SLSQP convergence (IFT-adjoint evaluations)
    axes[2].plot(
        np.arange(1, len(opt_history) + 1),
        opt_history,
        "o-",
        color=style.BLUE,
        markersize=4,
    ...
    save_json(
        "research_12_material_thickness_design_map",
        {
            "metadata": execution_metadata(),
            "bandgaps_eV": eg.tolist(),
            "thickness_um": thickness.tolist(),
            "efficiency_percent": values.tolist(),
            "start": x0.tolist(),
            "optimized": x_opt.tolist(),
            "pce_optimized_percent": pce_opt,
    ...
    best_sol = dj.simulate(
        device_from(x_opt, points, window),
        dj.Sweep(vmax=1.4, n_steps=steps),
        optics=dj.BeerLambert("tauc"),
    )

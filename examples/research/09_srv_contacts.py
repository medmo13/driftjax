...
def main():
    ...
    for snl in snls:
        dev = dj.Device(
            ...
            n_points=points, Snl=snl, Snr=1e7, Spl=1e7, Spr=1e7,
        )
    ...
    for key, label, color in [("voc_V", "$V_{oc}$ / V", style.SERIES[0]),
    ...
    for snl in snls:
        sol = sols[snl]
        plot_smooth_iv(ax1, sol.voltages, sol.current, color=None,
                       label=f"{snl:.0e}")
    ...
    save_json("research_09_srv_contacts", {
        "metadata": execution_metadata(),
        "n_points": points, "rows": rows, "figure": fp.name,
    })
    last = rows[-1]
    report("research_09", n_srvs=len(rows), voc_fast_V=last["voc_V"],

...
def main():
    ...
    def build_device(parameters):
        ...
        return dj.Device(n_points=points,
                         layers=[(t / 2, material, 10 ** float(log_doping)),
    ...
    axes[0].plot(
        [row[0][0] for row in history], [row[0][1] for row in history],
        "o-", color=style.BLUE, markersize=4, linewidth=1.2, alpha=0.8,
    )
    axes[0].plot(initial[0], initial[1], "s", color=style.GRAY, markersize=6, label="start", zorder=3)
    ...
    axes[0].set_title(f"Design trajectory: {len(history)} evals, {baseline*100:.1f}% → {optimized*100:.1f}%", fontsize=9, pad=8)
    axes[1].plot([row[1] * 100 for row in history], "o-", color=style.GREEN, markersize=4, linewidth=1.4)
    ...
    save_json("research_06_constrained_inverse_design", {
    ...
    baseline_sol = dj.simulate(build_device(np.asarray(history[0][0])), dj.Sweep(vmax=1.2, n_steps=steps))
    optimized_sol = dj.simulate(build_device(np.asarray(result.x)), dj.Sweep(vmax=1.2, n_steps=steps))

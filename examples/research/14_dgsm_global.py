...
BASE = dict(Chi=3.9, Eg=1.5, eps=9.4, Nc=8e17, Nv=1.8e19, Et=0, A=2e4, mn=100., mp=100., tn=1e-8, tp=1e-8)
...
def main():
    ...
    def eff_of(p):
        mv = eqx.tree_at(lambda mm: (mm.mn, mm.mp, mm.tn, mm.tp), mat0,
                         (p[0], p[1], 10**p[2], 10**p[3]))
        dev = dj.Device(n_points=npoints, layers=[(1e-4, mv, 1e17), (1e-4, mv, -1e17)],
    ...
    ax0, ax1 = axes
    ax0.bar(range(4), dgsm_n, color=["#0072B2","#E69F00","#009E73","#CC79A7"], edgecolor="black", linewidth=0.5, width=0.6)
    ax0.set_xticks(range(4), NAMES)
    ax0.set_ylabel(r"$S_i / \max S$ (DGSM)")
    ax0.set_ylim(0, 1.15)
    ax0.grid(axis="y", alpha=0.18, linestyle="--")
    ax0.set_title(rf"DGSM global (N={N} Sobol, $E[(\partial\eta/\partial p)^2]$)", fontsize=8, pad=8)
    ax1.bar(range(4), np.abs(g_loc)/np.max(np.abs(g_loc)), color=["#0072B2","#E69F00","#009E73","#CC79A7"], edgecolor="black", linewidth=0.5, width=0.6)

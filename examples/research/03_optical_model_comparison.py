...
def main():
    ...
    etl = dj.material(Eg=3.2, Chi=4.0, eps=9, Nc=1e19, Nv=1e19,
                      mn=20, mp=20, tn=1e-7, tp=1e-7, A=0)
    htl = dj.material(Eg=3.0, Chi=2.2, eps=3, Nc=1e19, Nv=1e19,
                      mn=20, mp=20, tn=1e-7, tp=1e-7, A=0)
    device = dj.Device(
        layers=[(5e-6, etl, 1e17),
                (4e-5, absorber, 0),
                (5e-6, htl, -1e17)],
        n_points= 500,
        Snl=1e7,
    ...
    solutions = {
        name: dj.simulate(device, dj.Sweep(vmax=1.2, n_steps= 61),
                          optics=optics)
    ...
    for idx, (name, solution) in enumerate(solutions.items()):
        ...
        axes[0].plot(solution.voltages, solution.current * 1e3, label=name, color=color, linewidth=1.6)
        # cell.G is dimensionless; G[cm^-3 s^-1] = G_dim * gratedens
        axes[1].plot(np.asarray(solution.cell.x) * float(length) * 1e4,
    ...
    shade_layers(axes[1], next(iter(solutions.values())),
                 labels=["ETL", "MAPbI$_3$ absorber", "HTL"], label_size=7)
    ...
    save_json("research_03_optical_model_comparison", {

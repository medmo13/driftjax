...
def main():
    ...
    material = dj.material(
        ...
        mn=1000., mp=300., tn=1e-6, tp=1e-6, A=3e4,
    )
    ...
    result = dj.optimize.nelder_mead(
        objective, start_um, bounds=bounds_um,
        maxiter=maxiter_opt,
    )
    ...
    sol_fit = dj.simulate(device_from(fitted_um), protocol_fine, solver=solver, optics=optics, adjoint=adjoint)
    sol_init = dj.simulate(device_from(jnp.asarray(start_um)), protocol_fine, solver=solver, optics=optics, adjoint=adjoint)

    fig, axes = new_figure(nrows=1, ncols=2, **DOUBLE_WIDE)
    # Clip the divergent post-Voc tail so the axes fit the physical window
    j_ref = max(abs(float(target_fine[0])), abs(float(reference[0])), abs(float(recovered[0]))) * 1e3
    clip = 1.3 * j_ref
    axes[0].plot(v_axis, np.clip(target_fine * 1e3, -clip, clip), color="black", lw=2, label="target")
    axes[0].plot(v_axis, np.clip(reference * 1e3, -clip, clip), "--", color=style.GRAY, linewidth=1.4, label="initial")
    axes[0].plot(v_axis, np.clip(recovered * 1e3, -clip, clip), "-", color=style.RED, linewidth=1.6, label="recovered")
    ...
    compare_metrics_box(axes[0], {"initial": sol_init, "recovered": sol_fit}, loc="lower left", fontsize=6.5)
    ...
    axes[1].semilogy(np.arange(len(history)), np.maximum(history, 1e-16), "o-", color=style.BLUE, markersize=4, linewidth=1.4)
    axes[1].set(xlabel="objective evaluation", ylabel="IV mean-squared error (A$^2$/cm$^4$)")
    axes[1].grid(which="both", alpha=0.18, linestyle="--")
    axes[1].set_title(f"MSE: {history[0]:.1e} → {history[-1]:.1e} ($\times${history[0]/max(history[-1],1e-16):.0f})", fontsize=9, pad=8)
    ...
    save_json("research_13_target_iv_structure", {
    ...
    print(f"RESEARCH_13_SCALED truth_um={float(truth_um[0]):.4f} fitted_um={float(fitted_um[0]):.6f} mse={float(result.fun):.3e} method=dense", flush=True)

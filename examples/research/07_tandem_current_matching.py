...
def main():
    ...
    top = dj.Device(n_points=points, layers=[(5e-6, top_material, 1e17),
    ...
    def si_cell(thickness):
        return dj.Device(
            n_points=points,
            layers=[(thickness / 2, bottom_material, 1e17),
    ...
    plot_smooth_iv(axes[0], top_solution.voltages, top_solution.current,
                   color=style.BLUE, label="top (perovskite)")
    plot_smooth_iv(axes[0], best_bottom.voltages, best_bottom.current,
                   color=style.RED,
    ...
    axes[0].legend(frameon=False, fontsize=7, ncol=1, handlelength=1.2,
                   loc="upper right")
    ...
    mpp_top_mW = float(top_solution.voc) * abs(float(top_solution.jsc)) * float(top_solution.ff) * 1e3
    ...
    axes[0].text(0.02, 0.02, fom_top + "\n" + fom_bot, transform=axes[0].transAxes, ha="left", va="bottom", fontsize=6, color="0.15",
    ...
    axes[1].plot(thicknesses * 1e6, mismatch * 1e3, "o-", color=style.BLUE,
                 label="J$_{sc}$ mismatch")
    axes[1].axhline(0, color="black", lw=0.7)
    axes[1].set(xlabel="bottom-cell thickness / $\\mu$m",
                ylabel="J$_{sc}$ mismatch / mA cm$^{-2}$")
    power_axis = axes[1].twinx()
    power_axis.plot(thicknesses * 1e6, tandem_eta * 100, "s--", color=style.RED,
                    label=r"ideal tandem $\eta$")
    ...
    axes[0].text(
        ...
        transform=axes[0].transAxes, ha="left", va="top", fontsize=6.5, color="0.15",
        bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="0.75", alpha=0.88),
    )
    tag_panels(axes)
    figure_path = save_figure(figure, "research_07_tandem_current_matching")
    save_json("research_07_tandem_current_matching", {
